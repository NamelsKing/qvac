#!/usr/bin/env bash
#
# Create a local Python venv at ./venv with the packages the .nemo ->
# .gguf converter needs (see scripts/requirements.txt). Idempotent:
# safe to re-run.
#
# Usage:
...
#
# The converter inside the venv is invoked via scripts/convert-nemo.sh,
# which auto-discovers ./venv/{bin,Scripts}/python before falling back
...
# On Windows under Git Bash, $PATH's `python3` is often the MSYS2 /
# UCRT Python (platform tag mingw_x86_64_ucrt_gnu) which has zero
# wheels on PyPI for numpy / torch / friends, so pip falls back to a
# source build and needs Ninja + a C++ toolchain just to install
# numpy. The Windows Python launcher (`py -3`) finds the real
# CPython (platform tag win-amd64) which has wheels for everything.
# Resolve `py -3` to its absolute path so the rest of this script can
# treat PYTHON_BIN as a single argv[0].
