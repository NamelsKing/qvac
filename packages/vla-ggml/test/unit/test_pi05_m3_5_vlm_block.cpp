...
TEST(Pi05M3_5, VlmBlock0MatchesPytorchOverValidPrefix) {
  ...
  const std::vector<float> expected = activations.readF32("vlm.blk_0.ffn_out");
  ...
  std::memcpy(
      x->data, prefix.data(), VALID_PREFIX_LEN * VLM_HIDDEN * sizeof(float));
  ...
  std::memcpy(pos->data, pos_data.data(), VALID_PREFIX_LEN * sizeof(int32_t));
  ...
  struct ggml_cgraph* gf =
      ggml_new_graph_custom(ctx_g, /*size=*/4096, /*grads=*/false);
