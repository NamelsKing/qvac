'use strict'
// Performance-recording helper for the tts-ggml (Chatterbox) integration
// tests. Holds the singleton perf-reporter, the mobile-safe inline fallback,
// and `recordTtsStats()` which turns an addon `response.stats` payload into a
// perf row.
//
...
function recordTtsStats (label, stats, extra) {
  ...
  const lines = [
    ...
    `    - RTF: ${rtf !== null ? rtf.toFixed(4) : 'n/a'}`,
