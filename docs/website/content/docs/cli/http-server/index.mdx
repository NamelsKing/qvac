...
* [Video generation](#videos) (text-to-video and image-to-video) — via `/v1/videos`.
...
<Callout type="info">
All multipart endpoints (`/v1/audio/*`, `/v1/images/edits`, `/v1/files`, `/v1/videos`) cap the request body at **100 MB**.
</Callout>
...
OpenAI-compatible **async** video generation — text-to-video (`txt2vid`) and image-to-video (`img2vid`), backed by the SDK's `video()`. Creating a job returns immediately with `status: "queued"`; the generation runs in the background. Poll for status, then download the bytes. The OpenAI sub-routes `/edits`, `/remix`, `/extensions`, and `/characters` are not implemented (the SDK exposes no underlying modes for them).
...
<Callout type="info">
**Image-to-video** additionally requires the loaded model to declare **`clipVisionModelSrc`** in its `config` (it accepts an SDK constant name, resolved by the registry like the other model-source fields). Text-to-video does not need it.
</Callout>

#### `POST /v1/videos`

Create a generation job. The endpoint accepts two content types:

- **`application/json`** — text-to-video (`txt2vid`).
- **`multipart/form-data`** — image-to-video (`img2vid`), selected by including an `init_image` file field. The mode is inferred from the presence of that image; there is no explicit `mode` field.
...
**Image-to-video (multipart, animates a first frame):**

```bash
curl http://localhost:11434/v1/videos \
  -F "model=wan-i2v" \
  -F "prompt=the subject slowly turns and smiles, cinematic lighting" \
  -F "init_image=@portrait.png" \
  -F "strength=0.85"
```
...
| `init_image` | **Multipart only.** First-frame image file (PNG or JPEG). When present, the job runs in `img2vid` mode; omit it for `txt2vid`. | No |
| `strength` | `img2vid` denoise strength in `[0, 1]` — how much the output may diverge from the first frame. Only meaningful with `init_image`. Out-of-range or non-numeric → `400 invalid_strength`. | No |
...
In `multipart/form-data` requests, the numeric fields (`fps`, `steps`, `seed`, `cfg_scale`, `flow_shift`, `strength`) may be sent as strings — they are coerced server-side.

<Callout type="info">
`img2vid` is selected by sending an `init_image` file; OpenAI's `input_reference` field is not used (it is ignored if present).
</Callout>
...
| 400 | `invalid_strength` | `strength` is outside `[0, 1]` or non-numeric. |
